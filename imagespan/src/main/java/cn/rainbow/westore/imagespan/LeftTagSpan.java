...
public class LeftTagSpan extends ReplacementSpan {
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
        ...
        mSpanWidth = Math.round(paint.measureText(text, start, end));
        Log.d("getSize: ","LeftTagSpan");
        return mSpanWidth;
    }
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        Log.d("draw: ","LeftTagSpan");

        RectF rect = new RectF(x, top, x+mRoundRadius*2, bottom);
        canvas.drawRoundRect(rect, mRoundRadius, mRoundRadius, mLeftTag);

        canvas.drawRect(x + mRoundRadius, top, x + mSpanWidth, bottom, mLeftTag);
